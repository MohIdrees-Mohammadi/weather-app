...
const CurrentWeather = () => {
  ...
  const getWeather = async () => {
    try {
      const url =
        "https://api.openweathermap.org/data/2.5/weather?q=herat&appid=e98b2575eec19db0cf6d6917d0618239&units=metric";
  ...
  return (
    <section className="flex w-full flex-col gap-6 px-4 pt-4 pb-24 sm:px-5 md:px-6 lg:h-full lg:min-h-0 lg:flex-row lg:gap-5 lg:overflow-hidden lg:px-4 lg:pt-0 lg:pb-3 xl:gap-6 xl:px-5">
      <div
        className={`flex flex-col lg:grid lg:w-[63%] lg:h-full lg:min-h-0 gap-6 md:gap-7 lg:gap-3 xl:gap-4 items-center lg:items-stretch text-gray-200 ${toggle ? "lg:[grid-template-rows:minmax(0,1fr)_minmax(0,0.95fr)_minmax(0,1fr)]" : "lg:[grid-template-rows:minmax(0,0.9fr)_minmax(0,1.7fr)]" }`}>
